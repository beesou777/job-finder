...
/**
 * Scrape JobAxle job detail page
 * Since we already have basic data from the API, this is mainly for getting full description
 */
export async function scrapeJobAxleDetail(url: string): Promise<JobData | null> {
  try {
    ...
    console.error(`Error scraping JobAxle detail: ${error.message}`);
    return null;
  }
}

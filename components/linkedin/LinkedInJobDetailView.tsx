...
/**
 * Server-rendered LinkedIn job detail. The page already fetched `job` on the
 * server, so this renders HTML directly — no `/api/linkedin-jobs/:id`
 * request in the browser Network tab, and scrapers only ever see a
 * truncated preview, never the full raw record as clean JSON.
 */
export function LinkedInJobDetailView({ job }: { job: any }) {
  if (!job) {
    return <p className="py-12 text-center text-red-600">Job not found.</p>;
  }

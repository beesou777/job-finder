    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage scraper configuration and system maintenance.
        </p>
      </div>
      ...
      <div className="grid gap-6">
        ...
        <Card className="border-l-4 border-l-blue-500">
          ...
          <CardContent className="space-y-4">
            ...
            {scrapeResult && (
              <div
                className={`p-4 rounded-md text-sm ${scrapeResult.success ? "bg-green-50 text-green-700 dark:bg-green-900/20" : "bg-red-50 text-red-700 dark:bg-red-900/20"}`}
              >
                <p className="font-bold mb-1">{scrapeResult.success ? "Success" : "Error"}</p>
                <pre className="whitespace-pre-wrap text-xs">
                  {JSON.stringify(scrapeResult.data || scrapeResult.error, null, 2)}
                </pre>
              </div>
            )}
          </CardContent>

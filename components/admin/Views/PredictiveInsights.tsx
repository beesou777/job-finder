...
export function PredictiveInsights({ forecast, indices, onWindowChange }: PredictiveProps) {
    if (!forecast || !indices) return <div className="animate-pulse h-32 bg-gray-100 rounded-md"></div>;
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white border-none relative overflow-hidden group">
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-slate-400 mt-2">Weighted score of volume, freshness & diversity.</p>
                    </CardContent>
                </Card>
                    ...
                    <CardContent>
                        <div className="text-3xl font-bold">{indices.remoteReadiness?.value}/100</div>
                         <div className="w-full bg-secondary h-2 rounded-full mt-3 overflow-hidden">
                            <div className="bg-yellow-500 h-full" style={{ width: `${indices.remoteReadiness?.value}%` }}></div>
                        </div>
                        <p className="text-[10px] text-muted-foreground mt-2 uppercase tracking-tighter">Tracks remote/hybrid job adoption</p>
                    </CardContent>
                </Card>
                 ...
                 <Card>
                    <CardHeader className="pb-2">
                         <CardTitle className="text-sm font-medium flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="text-[10px] h-5 py-0">v1.2-beta</Badge>
                         </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold flex items-center gap-2">
                            {forecast.insight === "Upward Trend" ? <TrendingUp className="text-green-500 h-5 w-5" /> : <TrendingDown className="text-red-500 h-5 w-5" />}
                            {forecast.insight}
                        </div>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-7">
                    <div>
                        ...
                        <CardDescription>AI-projected volume with <span className="text-primary font-semibold">95% confidence bands</span>.</CardDescription>
                    </div>
                ...
                <CardContent>
                    <div className="h-[350px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={forecast.forecast} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                <defs>

...
export function AdminSidebar({
  ...
  return (
    <div
      className={`flex flex-col h-screen bg-slate-950 text-slate-100 border-r border-slate-800 transition-all duration-300 ${isCollapsed ? "w-16" : "w-64"}`}
    >
      ...
      <div className="p-4 border-t border-slate-800">
        {!isCollapsed && (
          <div className="text-xs text-slate-500 text-center">v2.0.0 Admin Portal</div>
        )}
      </div>
    </div>
  );

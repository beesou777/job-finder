        ...
        <div className="rounded-2xl border border-white/10 bg-[#141416] p-6 md:p-8">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              ...
              <h2 className="text-2xl font-black md:text-3xl">What the current listings show</h2>
              <p className="mt-2 max-w-xl text-sm leading-6 text-zinc-400">
                A live platform snapshot to help you choose where to focus your search.
              </p>
            </div>
            <Link
              href="/insights"
              className="inline-flex items-center gap-2 font-black text-primary hover:text-white"
            >
              Full insights <ArrowUpRight className="h-4 w-4" />
            </Link>
          </div>
          <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {cards.map(([label, value, href], index) => (
              <Link
                key={label}
                href={href}
                className="rounded-xl border border-white/10 bg-zinc-950/70 p-4 transition hover:border-primary/60"
              >
                ...
                <p className="mt-3 text-2xl font-black">{value.toLocaleString("en-IN")}</p>
              </Link>
            ))}
          </div>
        </div>

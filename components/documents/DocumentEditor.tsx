...
function Editor({ id }: { id: string }) {
  ...
  async function trackApplication() {
    ...
    try {
      await createApplication({ jobRef: pack.job.ref, jobTitle: pack.job.title, company: pack.job.company || null, destinationUrl: pack.job.applyUrl, documentPackId: pack.id, documentVersion: pack.version });
      setNotice('This application is now in your tracker. Open the official page, submit there, then update the tracker when you are done.');
    } catch (cause: unknown) { setError(cause instanceof Error ? cause.message : 'Could not add this application to your tracker.'); }
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 text-foreground sm:px-8">
    ...
    <nav aria-label="Document progress" className="flex flex-wrap gap-3 text-sm">{['Choose approach', 'Suggestions', 'Edit', 'Layout', 'Review', 'Download'].map((label, index) => !tailored && index === 1 ? null : <span key={label} aria-current={step === index ? 'step' : undefined} className={step === index ? 'font-bold text-primary' : 'text-muted-foreground'}>{label}</span>)}</nav>
    {step === 0 && <section className="rounded-xl border bg-card p-6"><h2 className="text-2xl font-bold">How would you like to prepare your CV?</h2><p className="mt-2 text-muted-foreground">Use this saved draft or paste your own CV in the next step. Your text stays unchanged until you edit it.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><button type="button" className={button + ' flex-col items-start gap-2 p-6 text-left'} onClick={() => { setTailored(false); setStep(2); }}><span className="text-lg">Use my CV</span><span className="font-normal">Keep your wording, edit if needed, then choose a layout.</span></button><button type="button" className={button + ' flex-col items-start gap-2 p-6 text-left'} onClick={() => { setTailored(true); setStep(1); }}><span className="text-lg">Tailor for this job</span><span className="font-normal">Review suggestions, then improve the CV using your real experience.</span></button></div></section>}
    <div hidden={step !== 3 && step !== 5}><ResumeTemplates resume={content.resume} coverLetter={content.coverLetter} disabled={!reviewed || busy || blocked} selectionOnly={step === 3} /></div>
    <div className="space-y-6">

      <section hidden={step !== 2} className="min-w-0 space-y-5 rounded-xl border border-border bg-card p-5 sm:p-7" aria-labelledby="document-edit-heading">
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          ...
          {dirty && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
              <strong>Unsaved edits:</strong> Click &ldquo;Save changes&rdquo; in the editor on the left before you can confirm and mark this version as reviewed.
            </div>
          )}
          {changed && (
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs text-blue-900">
              <strong>Profile updated:</strong> Your career profile facts changed after this draft was generated. Create a fresh pack to review with updated facts.
            </div>
          )}
          {reviewed && (
            <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs font-medium text-emerald-900">
          ...
          <label className="flex items-start gap-3 text-sm leading-6">
            ...
            />
            <span className={reviewed ? 'font-medium text-emerald-900' : ''}>
          ...
          <button
            ...
            onClick={review}
            className={`${button} w-full ${reviewed ? 'bg-emerald-50 border-emerald-300 text-emerald-900 font-bold' : ''}`}
          >
          ...
          {reviewed && (
            <div className="border-t border-border pt-4">
              <h3 className="font-semibold text-zinc-900">Ready for the official application</h3>
    ...
    {step > 0 && <footer className="flex flex-wrap items-center justify-between gap-4 border-t pt-5"><button type="button" className={button} disabled={busy || tracking} onClick={() => setStep(step === 2 && !tailored ? 0 : step - 1)}>Back</button>
    {step === 2 && dirty && <p className="text-sm">Save changes before continuing.</p>}
    {step === 4 && !reviewed && <p className="text-sm">Confirm accuracy and mark this saved version as reviewed to continue.</p>}
    {step < 5 && <button type="button" className={button} disabled={busy || tracking || blocked || (step === 2 && dirty) || (step === 4 && !reviewed)} onClick={() => setStep(step + 1)}>Next: {['', 'edit your CV', 'choose a layout', 'review', 'download'][step]}</button>}
    {step === 5 && <Link className={button} href="/dashboard/applications">Open application tracker</Link>}</footer>}
  </main>;
}

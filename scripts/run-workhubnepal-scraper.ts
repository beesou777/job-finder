/**
 * Script to run WorkHub Nepal scraper only
 * Run with: npm run scrape:workhubnepal or yarn scrape:workhubnepal
 * 
 * This script specifically targets workhubnepal.com to:
...
async function runWorkHubNepalScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting WorkHub Nepal scraper...\n");
    console.log("📡 This will fetch all jobs from workhubnepal.com API\n");
    
    const allJobs = await scrapeSource("workhubnepal");
    ...
    console.log(`\n✅ Found ${allJobs.length} jobs from WorkHub Nepal`);
    ...
    console.log("\n✅ WorkHub Nepal scraping completed!");
    console.log(`\n📊 Results:`);
    ...
    console.error("❌ WorkHub Nepal scraping error:", error.message);
    console.error(error.stack);
    process.exit(1);
  }
}

runWorkHubNepalScraper();

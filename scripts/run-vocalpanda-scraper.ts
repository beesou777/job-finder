/**
 * Script to run vocalpanda scraper only
 * Run with: npm run scrape:vocalpanda or yarn scrape:vocalpanda
 * 
 * This script specifically targets vocalpanda.com to:
 * - Fetch all jobs via POST API
 * - Save to database
 */
...
async function runVocalPandaScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting VocalPanda scraper...\n");
    console.log("📡 This will fetch all jobs from vocalpanda.com via POST API\n");
    
    const allJobs = await scrapeSource("vocalpanda");

    if (allJobs.length === 0) {
      console.log("⚠️  No jobs found. This might indicate:");
      console.log("   1. The API structure has changed");
    ...
    console.log(`\n✅ Found ${allJobs.length} jobs from VocalPanda`);
    ...
    console.error("❌ VocalPanda scraping error:", error.message);
    console.error(error.stack);
    process.exit(1);
  }
}

runVocalPandaScraper();

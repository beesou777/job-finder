/**
 * Script to run internepal scraper only
 * Run with: npm run scrape:internepal or yarn scrape:internepal
 * 
 * This script specifically targets internepal.com.np to:
 * - Fetch all jobs from internships, jobs, fresher-jobs, and freelance listings
 * - Save to database
 */
...
async function runInternNepalScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting Intern Nepal scraper...\n");
    console.log("📡 This will fetch all jobs from internepal.com.np\n");
    
    const allJobs = await scrapeSource("internepal");
    ...
    if (allJobs.length === 0) {
      ...
      console.log("   4. Cloudflare protection is blocking the scraper");
      console.log("\n💡 Check the console logs above for any errors.");
      process.exit(0);
    }

    console.log(`\n✅ Found ${allJobs.length} jobs from Intern Nepal`);
    ...
    console.log("\n✅ Intern Nepal scraping completed!");
    console.log(`\n📊 Results:`);
    ...
    console.error("❌ Intern Nepal scraping error:", error.message);
    console.error(error.stack);
    process.exit(1);
  }
}

runInternNepalScraper();

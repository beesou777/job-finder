/**
 * Script to run froxjob scraper only
 * Run with: npm run scrape:froxjob or yarn scrape:froxjob
 * 
 * This script specifically targets froxjob.com to:
 * - Scrape all active jobs (only those with "X days left")
 * - Save to database
 */
...
async function runFroxjobScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting Froxjob scraper...\n");
    console.log("📡 This will scrape all active jobs from froxjob.com\n");
    console.log("ℹ️  Only jobs with 'X days left' will be scraped (expired jobs are skipped)\n");
    
    const allJobs = await scrapeSource("froxjob");
    ...
    console.log(`\n✅ Found ${allJobs.length} jobs from Froxjob`);
    ...
    console.error("❌ Froxjob scraping error:", error.message);

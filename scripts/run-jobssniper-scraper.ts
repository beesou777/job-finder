/**
 * Script to run jobssniper scraper only
 * Run with: npm run scrape:jobssniper or yarn scrape:jobssniper
 * 
 * This script specifically targets jobssniper.com to:
...
async function runJobSniperScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting JobSniper scraper...\n");
    console.log("📡 This will fetch all jobs from jobssniper.com\n");
    
    const allJobs = await scrapeSource("jobssniper");
    ...
    console.log(`\n✅ Found ${allJobs.length} jobs from JobSniper`);
    ...
    console.error("❌ JobSniper scraping error:", error.message);

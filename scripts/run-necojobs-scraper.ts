/**
 * Script to run necojobs scraper only
 * Run with: npm run scrape:necojobs or yarn scrape:necojobs
 * 
 * This script specifically targets necojobs.com.np to:
...
async function runNecojobsScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting Necojobs scraper...\n");
    console.log("📡 This will fetch all categories and jobs from necojobs.com.np\n");
    
    const allJobs = await scrapeSource("necojobs");
    ...
    console.log(`\n✅ Found ${allJobs.length} jobs from Necojobs`);
    ...
    console.error("❌ Necojobs scraping error:", error.message);

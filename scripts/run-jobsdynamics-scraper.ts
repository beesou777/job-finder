/**
 * Script to run JobsDynamics scraper only
 * Run with: npm run scrape:jobsdynamics or yarn scrape:jobsdynamics
 * 
 * This script specifically targets jobsdynamics.com to:
...
async function runJobsDynamicsScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting JobsDynamics scraper...\n");
    console.log("📡 This will fetch all jobs from jobsdynamics.com HTML listings\n");
    
    const allJobs = await scrapeSource("jobsdynamics");
    ...
    console.log(`\n✅ Found ${allJobs.length} jobs from JobsDynamics`);
    ...
    console.log("\n✅ JobsDynamics scraping completed!");
    console.log(`\n📊 Results:`);
    ...
    console.error("❌ JobsDynamics scraping error:", error.message);
    console.error(error.stack);
    process.exit(1);
  }
}

runJobsDynamicsScraper();

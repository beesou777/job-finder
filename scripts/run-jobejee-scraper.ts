/**
 * Script to run jobejee scraper only
 * Run with: npm run scrape:jobejee or yarn scrape:jobejee
 * 
 * This script specifically targets jobejee.com to:
...
async function runJobejeeScraper() {
  try {
    console.log("🔄 Initializing database connection...");
    const dataSource = await getDataSource();
    const jobRepository = dataSource.getRepository(Job);

    console.log("🕷️  Starting Jobejee scraper...\n");
    console.log("📡 This will fetch all jobs from jobejee.com API\n");
    
    const allJobs = await scrapeSource("jobejee");
    ...
    console.log(`\n✅ Found ${allJobs.length} jobs from Jobejee`);
    ...
    console.error("❌ Jobejee scraping error:", error.message);

...
export class AnalyticsService {
    ...
    static async getExecutiveOverview() {
        ...
        const remoteJobs = await jobRepo.createQueryBuilder("job")
            .where("LOWER(job.jobType) LIKE :remote OR LOWER(job.title) LIKE :remote", { remote: "%remote%" })
            .getCount();
        ...
        return {
            ...
            jobsLast30Days: { value: jobsLast30Days, delta: Number(calcDelta(jobsLast30Days, jobsPrev30Days).toFixed(2)) },
            activeJobs,
            expiredJobs,
            totalJobs,
    ...
    static async getGrowthTrends(days = 30) {
        ...
        const dailyStats = await jobRepo.createQueryBuilder("job")
    ...
    static async getSourceStats() {
        ...
        const sourceStats = await jobRepo.createQueryBuilder("job")
    ...
    static async getCategoryStats() {
        ...
        const categoryStats = await jobRepo.createQueryBuilder("job")
    ...
    static async getJobTypeStats() {
        ...
        const typeStats = await jobRepo.createQueryBuilder("job")
    ...
    static async getLocationStats() {
        ...
        const locationStats = await jobRepo.createQueryBuilder("job")
    ...
    static async getCompanyStats() {
        ...
        const companyStats = await jobRepo.createQueryBuilder("job")
    ...
    static async getSeoInsights() {
        ...
        const stopWords = new Set(["a", "an", "the", "in", "on", "at", "for", "to", "of", "and", "or", "with", "senior", "junior", "mid", "level", "developer", "engineer", "officer", "manager", "executive", "nepal", "kathmandu", "wanted", "hiring", "urgent", "needed", "vacancy"]);
    ...
    static async getForecast(days = 7, window = 7) {
        ...
        const lastKnownCount = historicalPoints.length > 0 ? parseInt(historicalPoints[historicalPoints.length - 1].count) : 10; // Default to 10 if none
    ...
    static async getMarketIndices() {
        ...
        const freshnessRatio = overview.activeJobs > 0 ? (overview.jobsLast7Days.value / overview.activeJobs) : 0;
    ...
    static async getCompanyIntentScores() {
        ...
        const scores = await jobRepo.createQueryBuilder("job")
            .select("job.company", "company")

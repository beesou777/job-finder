...
export async function POST(request: NextRequest) {
  ...
  if (!key)
    return NextResponse.json({ error: "Interview coach is not configured." }, { status: 503 });
  const body: any = await request.json().catch(() => null);
  if (!body?.role || !body?.position)
    return NextResponse.json({ error: "Role and position are required." }, { status: 400 });
  ...
  if (!key && bank.length > 0)
    return NextResponse.json({
      questions: bank.slice(0, count),
      limit: { minimum, maximum, enforcedMaximum: 15 },
    });
  ...
  if (!response.ok)
    return NextResponse.json(
      { error: "Could not prepare the interview questions." },
      { status: 502 },
    );
  const data: any = await response.json();
  try {
    const parsed = JSON.parse(data.candidates?.[0]?.content?.parts?.[0]?.text || "{}");
    const generated = (Array.isArray(parsed.questions) ? parsed.questions : []).map(
      (prompt: string) => ({ type: "open-ended", prompt }),
    );
    return NextResponse.json({
      questions: [...bank, ...generated].slice(0, count),
      limit: { minimum, maximum, enforcedMaximum: 15 },
    });
  } catch {
    return NextResponse.json({ error: "Gemini returned invalid questions." }, { status: 502 });
  }
}

  ...
  try {
    ...
    try {
      const enrichedResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/companies/enriched-from-jobs`,
        {
          cache: "no-store",
        },
      );
      if (enrichedResponse.ok) {
        ...
        if (enrichedData.success && enrichedData.data) {
          ...
          strongMatchesCount = enrichedData.data.filter(
            (c: any) => c.matchConfidence && c.matchConfidence >= 90,
          ).length;
        }
      }
    } catch (error) {
